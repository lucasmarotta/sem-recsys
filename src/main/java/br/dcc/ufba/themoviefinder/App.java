...
public class App extends SpringFXApplication
{
	...
	public void start(ViewStage viewStage) throws Exception
	{
		...
		//Gera tokens do filme pelo primeiro título contendo
		...
		*/
		...
		/*
		//Gera comparações com RLWS por tipo pré-definido
		consoleServices.generateRLWSByLodIds(Arrays.asList(
				new LodRelationId("Luiz_Inácio_Lula_da_Silva", "Curitiba"),
				new LodRelationId("Paraná_(state)", "Curitiba"),
				new LodRelationId("Media", "News_media")
		...
		*/
		...
		//Gera recomendações para os próximos 30 usuários offline que não possuem recomendações. Não salva as recomendações para o usuário
		//consoleServices.generateRecommendationsByOfflineUsers(recModel);

		//Gera recomendações por usuários. Como exemplo obtém os próximos 30 usuários offline, mas pode ser outro tipo de lista
		...
		//Atualiza (ou inclui) as recomendações por usuários. Defina runTwice como true caso seja para novos usuários e for do tipo RLWS, para miticar falhas durante
